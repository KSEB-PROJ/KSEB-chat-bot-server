...
def create_ppt_context(topic: str, combined_info: str) -> dict:
    ...
    **JSON 출력 예시:**
    ...
    try:
        ...
        corrected_response = correction_chain.invoke({"malformed_json": response.content})
        try:
            corrected_json_string = corrected_response.content.strip().replace("```json", "").replace("```", "").strip()
            print("   - 2.3b: JSON 자가 수정 성공.")
            return json.loads(corrected_json_string)
        except json.JSONDecodeError as e:
            print(f"Fatal: JSON 자가 수정 실패. \n오류: {e}\n수정된 내용: {corrected_response.content}")
            raise ValueError("AI가 PPT 구조를 생성하고 수정하는 데 모두 실패했습니다.") from e

# --- 최종 PPTX 파일 생성 로직 (v7) ---
def build_pptx_from_context(context: dict) -> str:
    """사용자가 정의한 커스텀 레이아웃을 찾아 PPT 문서를 생성하고, 맨 앞에 안내 슬라이드를 추가합니다."""
    template_dir = os.path.join(os.path.dirname(__file__), '..', '..', 'templates', 'pptx')
    ...
    body_ph = next((ph for ph in guide_slide.placeholders if ph.placeholder_format.idx > 0), None)
    if body_ph:
        tf = body_ph.text_frame
        tf.clear()
        tf.word_wrap = True
        p1 = tf.add_paragraph(); p1.text = "이 PPT의 배경, 로고, 글꼴 등 공통 디자인 요소는 '슬라이드 마스터'에서 관리됩니다."; p1.font.size = Pt(18)
        p2 = tf.add_paragraph(); p2.text = "배경 이미지나 로고 등을 수정/삭제하시려면, 이 파일의 슬라이드 마스터에서 직접 편집해야 합니다."; p2.font.size = Pt(16); p2.level = 1
        p3 = tf.add_paragraph(); p3.text = "수정 방법: [보기] 탭 → [슬라이드 마스터]"; p3.font.bold = True; p3.font.size = Pt(16); p3.level = 2
        p4 = tf.add_paragraph(); p4.text = "\n이 안내 슬라이드는 최종 발표 전에 삭제하시면 됩니다."; p4.font.italic = True; p4.font.size = Pt(14)
    ...
    if slide.placeholders and len(slide.placeholders) > 1:
        slide.placeholders[1].text = f"발표자: {context.get('presenter', '')}\n팀원: {context.get('team_members', '')}\n전공: {context.get('major', '')}"

    # 본문 슬라이드
    for s_data in context.get("slides", []):
        layout_name = s_data.get("layout", "Layout_Body")
        slide_layout = layout_map.get(layout_name, layout_map.get("Layout_Body", prs.slide_layouts[1]))
        slide = prs.slides.add_slide(slide_layout)
        if slide.shapes.title:
            slide.shapes.title.text = s_data.get('title', '')
        body_ph = next((ph for ph in slide.placeholders if ph.placeholder_format.idx > 0), None)
        if body_ph:
            tf = body_ph.text_frame
            tf.clear()
            tf.word_wrap = True
            content = s_data.get('content', '')
            content_lines = content if isinstance(content, list) else str(content).split('\n')
...
@tool
async def generate_ppt(topic: str, user_id: int, channel_id: int, jwt_token: str) -> str:
